...
const defaultNameConverter = <T>(id: Enum<T>, name: string) =>
  name.replace(/([A-z](?=[A-Z0-9]))/g, '$1').replace(/([0-9](?=[A-z]))/g, '$1');

export const createEnum = <T extends { [key in keyof T]: Enum<T> }>(
  self: T,
  nameConverter: (id: Enum<T>, name: string) => string = defaultNameConverter
) => {
